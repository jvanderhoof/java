...
action :install do
  ...
  if new_resource.bin_cmds
    new_resource.bin_cmds.each do |cmd|
      ...
      alternative_exists = shell_out("update-alternatives --display #{cmd} | grep #{alt_path}").exitstatus == 0 # run_command.
      ...
      if new_resource.default
        alternative_is_set = shell_out("update-alternatives --display #{cmd} | grep \"link currently points to #{alt_path}\"").exitstatus == 0 # .run_command
